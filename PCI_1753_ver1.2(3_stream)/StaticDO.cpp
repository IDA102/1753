...
void IMP() //Функция импульса для pin_name.
{
...
void IMP1() //Функция импульса для pin_name.
{
	ErrorCode ret = Success;
	InstantDoCtrl * instantDoCtrl = AdxInstantDoCtrlCreate();
	DeviceInformation devInfo(deviceDescription);
...
void IMP2() //Функция импульса для pin_name.
{
	ErrorCode ret = Success;
	InstantDoCtrl * instantDoCtrl = AdxInstantDoCtrlCreate();
	DeviceInformation devInfo(deviceDescription);
...
int main()
{
	...
	while (true)
	{
		...
		case 6:
			//----------------------6
			if (lcnt == 0) //Проверка на режим работы импульсного режима(при включ.имп.реж. ручной режим pin_name не работает!).
			{
